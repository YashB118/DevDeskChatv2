...
### Final deliverables
- [x] All admin panels render with live updates. (Sessions, assignments, users, feedback, global mute — `session:status` flips status badges + auto-refreshes QR; `chat:assigned`/`chat:unassigned` updates assignment cache; `user:updated` flips disabled/role; `feedback:new` prepends inbox.)
- [ ] Assignment changes propagate to the affected developer in real time (verified E2E). — DEFERRED to Phase 12 Playwright suite (E2E harness lands then).
- [x] Code split confirmed via bundle analyzer. (Admin chunk 24.97 KB raw / 6.54 KB gz — well under 120 KB per-route budget; entry 896.72 KB raw / 277.28 KB gz, +2 KB gz over Phase 8 despite admin-only code growth.)

> **Status: ✅ Complete** — Admin surface shipped under [frontend/src/features/{sessions,assignments,admin,feedback,mute}/](frontend/src/features/). Realtime contract extended in [events.contract.ts](frontend/src/realtime/events.contract.ts) with `session:status` / `user:updated` / `feedback:new` (mirrors backend `session:status`; the other two are forward-declared until backend ships them in matching order). [eventBus](frontend/src/realtime/eventBus.ts) extended with `sessions:status`/`users:updated`/`feedback:new` payloads.
>
> **Sessions** — [features/sessions/](frontend/src/features/sessions/). Zod types ([types.ts](frontend/src/features/sessions/types.ts)), API ([sessions.api.ts](frontend/src/features/sessions/api/sessions.api.ts)) covering `GET/POST/DELETE /api/sessions[/:name]` + `/start` + `/stop` + `/qr`. Hooks: [useSessions / useSessionMutations](frontend/src/features/sessions/hooks/useSessions.ts), [useSessionQR](frontend/src/features/sessions/hooks/useSessionQR.ts) (enabled iff status === `SCAN_QR_CODE`, auto-invalidates on `sessions:status` event-bus flip). Pure cache-mutation in [sessions.mutations.ts](frontend/src/features/sessions/sync/sessions.mutations.ts); socket registration in [sessions.sync.ts](frontend/src/features/sessions/sync/sessions.sync.ts) (Zod-validated `session:status` → `setQueryData` + `eventBus.emit('sessions:status', …)`). Components: [SessionsPanel](frontend/src/features/sessions/components/SessionsPanel/SessionsPanel.tsx) (create/start/stop/delete + inline QR drawer), [SessionStatusBadge](frontend/src/features/sessions/components/SessionStatusBadge/SessionStatusBadge.tsx), [QRPanel](frontend/src/features/sessions/components/QRPanel/QRPanel.tsx) (base64 SVG `<img>` from `/api/sessions/:name/qr`), [CreateSessionDialog](frontend/src/features/sessions/components/CreateSessionDialog/CreateSessionDialog.tsx) (RHF + zodResolver).
>
> **Assignments** — [features/assignments/](frontend/src/features/assignments/). Cursor-paginated `useInfiniteQuery(keys.assignments())` via [useAssignments](frontend/src/features/assignments/hooks/useAssignments.ts); optimistic [useAssignmentActions](frontend/src/features/assignments/hooks/useAssignments.ts) with snapshot rollback per query key. Pure mutations in [assignments.mutations.ts](frontend/src/features/assignments/sync/assignments.mutations.ts); socket registration in [assignments.sync.ts](frontend/src/features/assignments/sync/assignments.sync.ts) ( `chat:assigned`/`chat:unassigned` → cache mutate, never refetch). [AssignmentsPanel](frontend/src/features/assignments/components/AssignmentsPanel/AssignmentsPanel.tsx) is virtualized via `react-virtuoso` w/ stable `computeItemKey={chatId}`, inline `<select>` reassignment + dedicated Unassign button.
>
> **Admin (users)** — [features/admin/](frontend/src/features/admin/). [adminApi](frontend/src/features/admin/api/admin.api.ts) covers `GET/POST/PATCH/DELETE /api/users`. [useUsers / useUserActions](frontend/src/features/admin/hooks/useUsers.ts) hook into `keys.users()`; [admin.sync.ts](frontend/src/features/admin/sync/admin.sync.ts) handles `user:updated` (flips `disabled` / `role` in cache). [DeveloperManagementPanel](frontend/src/features/admin/components/DeveloperManagementPanel/DeveloperManagementPanel.tsx) lists users with role badge + enabled toggle + delete; [CreateUserDialog](frontend/src/features/admin/components/CreateUserDialog/CreateUserDialog.tsx) uses RHF + zodResolver.
>
> **Feedback** — [features/feedback/](frontend/src/features/feedback/). [useFeedback](frontend/src/features/feedback/hooks/useFeedback.ts) wraps `useInfiniteQuery(keys.feedback())` + optimistic `markRead` w/ snapshot rollback. Pure helpers in [feedback.mutations.ts](frontend/src/features/feedback/sync/feedback.mutations.ts); socket [feedback.sync.ts](frontend/src/features/feedback/sync/feedback.sync.ts) listens to `feedback:new` and pulls the latest item via a tiny `list({ limit: 1 })` refetch to materialize the full body without forcing a full first-page invalidation. [FeedbackPanel](frontend/src/features/feedback/components/FeedbackPanel/FeedbackPanel.tsx) renders the list w/ "New" badge + Mark-read + Load-more.
>
> **Mute (global)** — [features/mute/](frontend/src/features/mute/). [muteApi](frontend/src/features/mute/api/mute.api.ts) hits `GET/PATCH /api/mute/global`. [useGlobalMute / useGlobalMuteActions](frontend/src/features/mute/hooks/useGlobalMute.ts) optimistic flip + rollback. [GlobalMuteToggle](frontend/src/features/mute/components/GlobalMuteToggle/GlobalMuteToggle.tsx) is a Switch + toast on success/failure, mounted inside [UsersPage](frontend/src/app/router/pages/admin/UsersPage.tsx). The notifications service that consumes this flag lands in Phase 10.
>
> **Cross-feature plumbing** — Boundaries plugin disallows `feature → feature` (except `${from.name}`). The admin-side directory dropdown (needed by AssignmentsPanel to render the developer `<select>`) is exposed as a `lib`-tier helper at [lib/data/useDirectory.ts](frontend/src/lib/data/useDirectory.ts) keyed by `keys.users()` — same cache slot as the admin feature's `useUsers`, so prefetching either side hydrates the other without crossing feature boundaries. Reusable destructive confirm UI promoted out of sessions into the design system at [design-system/compounds/ConfirmDialog/](frontend/src/design-system/compounds/ConfirmDialog/ConfirmDialog.tsx).
>
> **Pages** — Stubs replaced: [SessionsPage](frontend/src/app/router/pages/admin/SessionsPage.tsx), [AssignmentsPage](frontend/src/app/router/pages/admin/AssignmentsPage.tsx), [UsersPage](frontend/src/app/router/pages/admin/UsersPage.tsx) (DeveloperManagementPanel + GlobalMuteToggle), [FeedbackPage](frontend/src/app/router/pages/admin/FeedbackPage.tsx). All admin pages remain code-split via [lazyRoutes.ts](frontend/src/app/router/lazyRoutes.ts) + `lazy(import('./pages/admin'))` in [AppRouter](frontend/src/app/router/AppRouter.tsx); build manifest confirms a single dedicated admin chunk (`assets/index-*.js` 24.97 KB raw / 6.54 KB gz, well under the 120 KB per-route budget) separate from the entry chunk.
>
> **Sync registration** — [app/sync/featureSync.ts](frontend/src/app/sync/featureSync.ts) now registers `registerChatsSync` + `registerMessagesSync` + `registerSessionsSync` + `registerAssignmentsSync` + `registerAdminSync` + `registerFeedbackSync` at boot via [App.tsx](frontend/src/App.tsx).
>
> **Tests** — 152 pass / 40 files (+15 vs Phase 8): 3 `applySessionStatus` pure cases, 2 MSW-backed `useSessions` + `useSessionMutations` cases (list parse, create → invalidate), 3 `applyAssigned`/`applyUnassigned` pure cases, 3 `applyUserUpdated` pure cases, 3 feedback pure-mutation cases (markRead, prepend, dedupe), 1 MSW-backed `useGlobalMute` optimistic-rollback case. [AppRouter.test.tsx](frontend/src/app/router/AppRouter.test.tsx) updated to wrap in `ToastProvider` + MSW-stub `/api/sessions` for the `/admin/sessions` route. Lint + typecheck + build clean. Bundle: entry 896.72 KB raw / 277.28 KB gz (+2 KB gz over Phase 8 despite the admin chunk now carrying real code); admin chunk 24.97 KB raw / 6.54 KB gz.
>
> **Deferred** — Playwright E2E flow (admin creates session → developer sees live assignment) belongs in Phase 12 alongside the broader E2E harness. Confirmation flow for sensitive forms (password reentry on role change) deferred — admin → admin promotion is rare enough to defer to UX pass in Phase 10. Backend modules for `/api/assignments`, `/api/users` (CRUD), `/api/feedback`, `/api/mute/global` aren't built yet; frontend ships against the documented contract and Zod-fails closed if responses drift.
