...
| **9** | ✅ Complete | Collaboration modules: assignments, mute, feedback, users |
...
### Final deliverables
- [x] Phase-8 visibility stubs replaced with real assignment-driven filters.
- [x] All socket events for assignment present and tested.
- [x] Mute respected by notification path (foreshadowing Phase 10).
- [x] Feedback submit/list/read endpoints functional.

**Status: ✅ Complete** — implemented in [backend/src/modules/users/](backend/src/modules/users/), [backend/src/modules/assignments/](backend/src/modules/assignments/), [backend/src/modules/mute/](backend/src/modules/mute/), and [backend/src/modules/feedback/](backend/src/modules/feedback/). 217 tests pass; lint + typecheck + build green. Migration `0004_collaboration.ts` ships the `assignment_event` enum, `developer_assignments` (with partial-unique `(user_id, chat_id) WHERE is_active = true`), `assignment_history`, `chat_mutes`, `global_mutes`, `feedback`, plus their indexes. `UsersModule` gains `UsersAdminController` (`/api/admin/users`) with create/update/disable/enable/password-reset/list under `@UseGuards(JwtAuthGuard, AdminGuard)`; disable + password-reset run inside a single `TransactionRunner.run` that flips `disabled`, revokes every refresh-token family via `AuthRepository.revokeAllForUser`, writes the audit row, then severs live sockets through `SocketEmitter.disconnectUser` outside the transaction. `AssignmentsModule` (`/api/admin/assignments`) wraps insert+history in `withTransaction` and emits `chat:assigned` / `chat:unassigned` to both the developer's `user:<id>` room and `admin` via `SocketEmitter`. `MuteModule` (`/api/mute`) exposes per-chat + global toggles; the developer path is gated by `AssignmentRepository.findActive` so a dev can only mute chats actively assigned to them. `FeedbackModule` (`/api/feedback`) keeps developer visibility scoped to their own user id, while admins see everything and own the mark-read transition. `ChatPolicy` is now driven by `AssignmentRepository` — visibility/read/write all gate non-admins through active assignment rows — and `ChatsService.list` enriches each chat's `muted` flag via `MuteService.filterMutedChatIds`. `AuditEvent` union expands to cover `user.create / user.update / user.disable / user.enable / user.password.reset / assignment.create / assignment.remove / mute.chat.set / mute.global.set`. New `chat:assigned` / `chat:unassigned` schemas are registered in `events.contract.ts`. Live Testcontainers Postgres coverage for the partial-unique constraint, assignment-driven visibility, and the disabled-user socket eviction lands with the rest of the live-infra e2e suite in Phase 12.
...
### Final deliverables
- [x] `/metrics` returns Prom-formatted text with the documented metric set.
- [x] Traces in the configured OTLP collector connect HTTP requests to downstream queue jobs and WAHA calls.
- [x] Audit log entries for every sensitive action.

**Status: ✅ Complete** — implemented in [backend/src/config/telemetry.ts](backend/src/config/telemetry.ts), [backend/src/shared/observability/](backend/src/shared/observability/), and [backend/src/infra/health/](backend/src/infra/health/). 227 tests pass; lint + typecheck + build green. `startTelemetry()` runs at the very top of `main.ts` before any other import — when `OTEL_ENABLED=true`, the `@opentelemetry/sdk-node` boot dynamically pulls in `auto-instrumentations-node` (with `fs` opted out), wires `OTLPTraceExporter` against `OTEL_EXPORTER_OTLP_ENDPOINT`, and applies `TraceIdRatioBasedSampler(OTEL_TRACES_SAMPLER_RATIO)`. The handle is shutdown on `SIGTERM`/`SIGINT`. `MetricsModule` is `@Global()`; `MetricsController` (`@Public()`) gates `/metrics` behind `METRICS_BEARER_TOKEN` — endpoint is intentionally inaccessible when the token is unset. `metricsRegistry` carries the full prom-client surface: `http_request_duration_seconds`, `http_requests_total`, `socket_events_emitted_total`, `active_socket_connections`, `queue_job_duration_seconds`, `queue_jobs_total`, `waha_request_duration_seconds`, `waha_requests_total`, `waha_circuit_state`, `cache_lookups_total`, plus `postgres_pool_active_connections` / `postgres_pool_idle_connections` / `postgres_pool_waiting_clients` (sampled every 5s by `PostgresPoolCollector` against the pg pool master handle). `MetricsInterceptor` is registered globally and records duration + counter per matched route pattern (never raw path, to avoid cardinality blow-up). `WorkerHarness` records `(queue, jobName, outcome)` on every BullMQ run; `SocketEmitter` increments per outbound event keyed by `roomKindFor(room)`; `RealtimeGateway` inc/decs the socket gauge on connect/disconnect; `WahaService.instrument` records every call + projects circuit state (0=closed, 1=half-open, 2=open); `CacheService.wrap` records hit/miss per key namespace. `@TraceMethod()` decorator wraps service methods in an OTel span when traces are turned on. `HealthModule` extended with `WahaHealthIndicator` (cached + timeout-bounded `listSessions` probe) so `/health/ready` now reflects Postgres + Redis + WAHA reachability accurately. `AuditEvent` union expanded with `session.create/start/stop/delete`; `SessionsService.create/start/stop/delete` accept an optional `actorId` and write the audit row (`SessionsController` plumbs `@CurrentUser()` through). Live OTLP-collector + Prometheus scraper integration coverage lands with the rest of the live-infra suite in Phase 12.
...
### Final deliverables
- [x] All endpoint classes from [BACKEND_ARCHITECTURE.md §15](BACKEND_ARCHITECTURE.md) covered.
- [x] CSP and security headers verified (unit-asserted in [test/security.e2e-spec.ts](backend/test/security.e2e-spec.ts); a securityheaders.com sweep against the deployed staging hostname lands with the rest of Phase 12).
- [x] Dependency audit gated in CI (`npm audit --omit=dev --audit-level=critical` fails the build; `--audit-level=high` runs as advisory until the NestJS 10 → 11 bump unwinds the transitive `multer` highs).

**Status: ✅ Complete** — implemented in [backend/src/common/rate-limit/](backend/src/common/rate-limit/), [backend/src/common/interceptors/request-timeout.interceptor.ts](backend/src/common/interceptors/request-timeout.interceptor.ts), [backend/src/shared/errors/rate-limited.error.ts](backend/src/shared/errors/rate-limited.error.ts), [backend/src/shared/errors/request-timeout.error.ts](backend/src/shared/errors/request-timeout.error.ts), and the helmet/CI updates in [backend/src/main.ts](backend/src/main.ts) + [.github/workflows/ci.yml](.github/workflows/ci.yml). 247 tests pass (+20 vs Phase 10's 227); lint + typecheck + build green. `RateLimitService` is a Redis sliding-window counter backed by a Lua script (`zremrangebyscore` → `zcard` → `zadd` + `pexpire`) with `SCRIPT LOAD` + `evalsha` caching, `NOSCRIPT` recovery, and a fail-open branch so a Redis blip never black-holes the API. The factory landed as **two interceptor providers** (rather than the spec's `@UseGuards` shape) so soft mode can short-circuit cleanly with a cached body via rxjs `of(...)`: `GlobalRateLimitInterceptor` (APP_INTERCEPTOR) enforces the IP floor (`RATE_LIMIT_IP_*`, default 600/min) and the per-authenticated-user floor (`RATE_LIMIT_USER_*`, default 300/min); `RouteRateLimitInterceptor` (APP_INTERCEPTOR) reads `@RateLimit({preset, mode, identify, softCache})` metadata at the handler/controller. Three presets pull their caps from env at runtime: `auth` (5/15min by email on `POST /api/auth/login`), `send` (30/10s per user on `POST /api/messages/:chatId/send|media|forward`), `chats` (soft 10/5s per user on `GET /api/chats` — serves the existing 10s `CacheService.wrap` payload with `X-RateLimit-Cached: true`, degrades to a no-op when the cache misses). Hard 429s set `Retry-After` (seconds) + `X-RateLimit-Limit` / `X-RateLimit-Remaining`. `RateLimitedError` ships under code `RATE_LIMITED`; the same status code path already exists in `AllExceptionsFilter`. `RequestTimeoutInterceptor` (APP_INTERCEPTOR) wraps every HTTP handler observable in `timeout({ each: REQUEST_TIMEOUT_MS })` (default 30000); on fire it emits `RequestTimeoutError` (503 `REQUEST_TIMEOUT`). Helmet posture in `main.ts` switched to a strict API-only stance: `default-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'none'`, HSTS `max-age=63072000; includeSubDomains; preload`, `referrer-policy=no-referrer`, COOP `same-origin`, CORP `same-site`, `X-Permitted-Cross-Domain-Policies: none`, plus an explicit `Permissions-Policy` middleware denying camera/mic/geo/payment/clipboard/etc. New env surface: `RATE_LIMIT_ENABLED`, `REQUEST_TIMEOUT_MS`, `RATE_LIMIT_{IP,USER,AUTH,SEND,CHATS}_{WINDOW_SECONDS,MAX}`, `HSTS_MAX_AGE_SECONDS`. New tests: `rate-limit.service.spec.ts` (sliding window + window slide + per-key isolation + Redis-down fail-open), `route-rate-limit.interceptor.spec.ts` (no-descriptor passthrough, hard 429 + Retry-After, soft hit serves cached, soft miss degrades, env-disabled noop), `global-rate-limit.interceptor.spec.ts` (IP-only when unauthed, IP + user when authed, Retry-After on IP block, env-disabled noop), `request-timeout.interceptor.spec.ts` (fast passthrough, timeout → 503, handler-error passthrough, non-HTTP skip, error envelope sanity), and `test/security.e2e-spec.ts` (helmet headers actually emitted, slow handler returns the normalized 503 envelope). Cross-pod shared-counter behaviour falls out of the Redis-backed implementation; live two-process verification against Testcontainers Redis lands with the rest of the live-infra suite in Phase 12.
