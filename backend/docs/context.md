...
- Collaboration: assignments, mute, feedback, admin user CRUD (✅ Phase 9).
- Observability: metrics, traces, health, broadened audit (✅ Phase 10).
- Security hardening: layered Redis-backed rate limits, 30s request budget, strict API-only helmet posture (✅ Phase 11).

Completed phases of `BACKEND_IMPLEMENTATION_PLAN.md` today: **Phase 1 — Foundation**, **Phase 2 — Persistence Layer**, **Phase 3 — Authentication**, **Phase 4 — Real-time Core**, **Phase 5 — Queue Infrastructure**, **Phase 6 — External Integration: WAHA + SQLite Store**, **Phase 7 — Webhook Ingestion**, **Phase 8 — Domain Modules**, **Phase 9 — Collaboration**, **Phase 10 — Observability**, **Phase 11 — Security Hardening + Rate Limiting**. The server boots, parses env, connects to PostgreSQL via TypeORM and Redis via ioredis, exposes terminus-driven liveness + readiness probes, signs RS256 JWTs, rotates opaque refresh tokens with family-scoped reuse detection, writes append-only audit entries, accepts authenticated Socket.IO connections with Redis-adapter fan-out and a typed event contract, drives BullMQ queues through a Zod-validated worker harness, talks to WAHA through a per-method-circuit-breaker / TTL-cached / retried client, reads the NOWEB SQLite store read-only with per-session handles, ingests WAHA webhooks (HMAC-verified, immediately enqueued, normalized phone→LID, dispatched by event type), persists chats / messages / sessions with monthly-partitioned `messages`, reconciles outbound sends against the pending-message store, emits typed `message:*` / `session:status` / `group:participants` events, exposes admin user CRUD (with single-transaction disable that revokes refresh-token families + severs live sockets), enforces assignment-driven chat visibility, emits typed `chat:assigned` / `chat:unassigned` events, owns per-chat + global mute toggles surfaced in chat-list enrichment, gates the feedback inbox by role, applies a Redis sliding-window rate-limit floor (IP + per-user) plus per-route caps (auth-login, message-send, soft chat-list), enforces a 30s per-request time budget, and serves the API behind a strict `default-src 'none'` helmet posture. All non-health HTTP routes sit under the `/api` global prefix.
...
│   ├── modules/
│   │   ├── users/                    UsersModule (Phase 3 + 9): User entity + UserRepository + admin CRUD (`/api/admin/users`); disable/enable + admin password reset run inside a single `withTransaction` that revokes refresh-token families and severs live sockets via `SocketEmitter.disconnectUser`.
│   │   ├── auth/                     AuthModule: login/refresh/logout/password-change, RS256 JWT, refresh rotation + family revocation, audit_log writes (Phase 9 expanded the AuditEvent union for user/assignment/mute events).
│   │   ├── assignments/              AssignmentsModule (Phase 9): admin-only `/api/admin/assignments`; `withTransaction` insert/deactivate + history; partial-unique `(user_id, chat_id) WHERE is_active = true` prevents duplicates; emits `chat:assigned` / `chat:unassigned` to `user:<dev>` + `admin`.
│   │   ├── mute/                     MuteModule (Phase 9): per-chat + global mute (`/api/mute`); developers can only mute chats actively assigned to them; admin path is unrestricted.
│   │   ├── feedback/                 FeedbackModule (Phase 9): submit/list/mark-read; admins see all feedback, developers only their own; admin owns the read transition.
...
│   │   ├── chats/                    ChatsModule (Phase 8 + 9): chat list (10s Redis cache via CacheService.wrap), dedup by id, ChatMetadataRepository, ChatPolicy gated by `developer_assignments`; chat-list enrichment fills `muted` from `MuteService.filterMutedChatIds`.
...
│   ├── shared/observability/         MetricsModule (Phase 10): prom-client registry, `/metrics` (token-gated), HTTP MetricsInterceptor (global), PostgresPoolCollector, `@TraceMethod` decorator.
...
`app.module.ts` imports — in order — `ConfigModule`, `LoggerModule`, `DatabaseModule`, `CacheModule`, `MetricsModule`, `RateLimitModule`, `HealthModule`, `UsersModule`, `AuthModule`, `RealtimeModule`, `WahaModule`, `WahaStoreModule`, `SessionsModule`, `MessagesModule`, `AssignmentsModule`, `MuteModule`, `ChatsModule`, `FeedbackModule`, `WebhooksModule`, then `QueueModule`. `UsersModule` and `AuthModule` resolve their mutual dependency via `forwardRef` (UsersService injects `AuthRepository` for token-family revocation and audit writes). `TransactionRunner` is provided locally inside each module that needs it. It implements `NestModule.configure` to attach `CorrelationMiddleware` for every route. Three APP_INTERCEPTOR providers are registered globally: `GlobalRateLimitInterceptor` (IP + per-user floors) and `RouteRateLimitInterceptor` (descriptor-driven, supports hard 429 + soft cache fallback) from `RateLimitModule`, plus `RequestTimeoutInterceptor` (30s budget by default, surfaces `RequestTimeoutError`/503) provided directly inside `AppModule`. Authentication is still enforced per-controller via `@UseGuards(JwtAuthGuard)` with a `@Public()` opt-out for the login/refresh endpoints and the webhook ingress (the `@Public()` metadata is honoured by `JwtAuthGuard` itself).
...
- Mounts the strict Phase-11 `helmet()` config (`default-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'none'`, HSTS configured from `HSTS_MAX_AGE_SECONDS`, `Referrer-Policy: no-referrer`, COOP `same-origin`, CORP `same-site`, `X-Permitted-Cross-Domain-Policies: none`) followed by a `Permissions-Policy` middleware that denies camera, microphone, geolocation, payment, clipboard, USB, MIDI, sensors, fullscreen, autoplay, display-capture, then `cookieParser()` (cookie-parser must run before any handler reads `req.cookies` — the auth controller depends on it for the refresh cookie).
...
Phase 2 added `DATABASE_URL`, `PG_POOL_MAX`, `PG_STATEMENT_TIMEOUT_MS`, `PG_IDLE_IN_TX_TIMEOUT_MS`, `PG_SSL`, `PG_SSL_REJECT_UNAUTHORIZED`, `PG_SSL_CA`, `REDIS_URL`, and `REDIS_KEY_PREFIX` to the schema. Phase 3 added the auth surface: `JWT_PRIVATE_KEY`, `JWT_PUBLIC_KEY`, `JWT_ACCESS_TTL_SECONDS`, `JWT_ISSUER`, `JWT_AUDIENCE`, `REFRESH_TTL_DAYS`, `REFRESH_COOKIE_NAME`, `REFRESH_COOKIE_PATH`, `REFRESH_COOKIE_SECURE`, `REFRESH_COOKIE_DOMAIN`, `BCRYPT_COST`. Phase 5 added the queue surface: `QUEUE_PREFIX`, `QUEUE_DEFAULT_ATTEMPTS`, `QUEUE_DEFAULT_BACKOFF_MS`, `QUEUE_REMOVE_ON_COMPLETE`, `QUEUE_REMOVE_ON_FAIL`. Phase 6 added the WAHA HTTP + SQLite surface: `WAHA_BASE_URL` (required), `WAHA_API_KEY?`, `WAHA_TIMEOUT_MS`, `WAHA_MEDIA_TIMEOUT_MS`, `WAHA_RETRY_MAX`, `WAHA_RETRY_BASE_MS`, `WAHA_CB_FAILURE_THRESHOLD`, `WAHA_CB_COOLDOWN_MS`, `WAHA_SESSIONS_CACHE_TTL_MS`, `WAHA_CHATS_CACHE_TTL_MS`, `WAHA_STATUS_CACHE_TTL_MS`, `WAHA_STORE_PATH` (required), `WAHA_STORE_REQUIRE_READONLY`, `WAHA_STORE_CACHE_TTL_MS`. Phase 7 added the webhook surface: `WAHA_WEBHOOK_HMAC_SECRET?`, `WAHA_WEBHOOK_HMAC_HEADER`, `PENDING_MESSAGE_TTL_MS`. Phase 10 added the observability surface: `OTEL_ENABLED`, `OTEL_SERVICE_NAME`, `OTEL_EXPORTER_OTLP_ENDPOINT?`, `OTEL_TRACES_SAMPLER_RATIO`, `METRICS_BEARER_TOKEN?`, `HEALTH_WAHA_CACHE_TTL_MS`, `HEALTH_WAHA_TIMEOUT_MS`. Phase 11 added the hardening surface: `RATE_LIMIT_ENABLED`, `REQUEST_TIMEOUT_MS`, `RATE_LIMIT_IP_{WINDOW_SECONDS,MAX}`, `RATE_LIMIT_USER_{WINDOW_SECONDS,MAX}`, `RATE_LIMIT_AUTH_{WINDOW_SECONDS,MAX}`, `RATE_LIMIT_SEND_{WINDOW_SECONDS,MAX}`, `RATE_LIMIT_CHATS_{WINDOW_SECONDS,MAX}`, `HSTS_MAX_AGE_SECONDS`. PEM keys accept literal `\n` escapes and are normalized to real newlines by the loader. See [modules/config.md](modules/config.md) for the full schema and behaviour.

## 8. Observability baseline

Single logger, one process. Production = line-delimited JSON. Development = `pino-pretty`. Every log record carries `correlationId` when an HTTP request is in flight (via the child logger bound in `CorrelationMiddleware`). Health endpoints are excluded from `pino-http`'s auto-logging to keep noise out of dashboards.

**Phase 10 surface.** OpenTelemetry initializes at the very top of `main.ts` via `startTelemetry()` (governed by `OTEL_ENABLED`); on activation it dynamically pulls `@opentelemetry/sdk-node`, registers `auto-instrumentations-node` (with `fs` opted out), and wires an OTLP HTTP trace exporter against `OTEL_EXPORTER_OTLP_ENDPOINT`. `TraceIdRatioBasedSampler(OTEL_TRACES_SAMPLER_RATIO)` controls sampling. The SDK shuts down cleanly on `SIGTERM` / `SIGINT`. Prometheus runs from a single registry exposed at `GET /metrics` (token-gated via `METRICS_BEARER_TOKEN` — when unset the endpoint refuses every request). The metric surface: `http_request_duration_seconds`, `http_requests_total`, `socket_events_emitted_total`, `active_socket_connections`, `queue_job_duration_seconds`, `queue_jobs_total`, `waha_request_duration_seconds`, `waha_requests_total`, `waha_circuit_state` (0=closed, 1=half-open, 2=open), `cache_lookups_total`, `postgres_pool_active_connections`, `postgres_pool_idle_connections`, `postgres_pool_waiting_clients`. The HTTP histogram is fed by a global `MetricsInterceptor` (uses matched route patterns to avoid cardinality blow-up); the pool gauges are scraped every 5s by `PostgresPoolCollector` against the `pg` pool's master handle. Use `@TraceMethod()` from `shared/observability/trace-method.decorator.ts` to wrap service methods in custom spans when the auto-instrumented HTTP / DB / queue boundaries aren't enough.

The terminus readiness payload (`{ status, info, error, details }`) follows the upstream `@nestjs/terminus` shape rather than the app's standard `{ status: "ok", ... }` liveness shape — clients must read both. `/health/ready` probes Postgres + Redis + WAHA (the WAHA probe caches its last result for `HEALTH_WAHA_CACHE_TTL_MS` so a fast scrape loop doesn't hammer the upstream, and a `HEALTH_WAHA_TIMEOUT_MS` race bounds the worst case).
...
- **Auth (Phase 3):**
  ...
  - Append-only `audit_log` writes for `auth.login.success`, `auth.login.failure`, `auth.refresh.success`, `auth.refresh.reuse`, `auth.refresh.invalid`, `auth.logout`, `auth.password.change`, plus Phase 9: `user.create`, `user.update`, `user.disable`, `user.enable`, `user.password.reset`, `assignment.create`, `assignment.remove`, `mute.chat.set`, `mute.global.set`, plus Phase 10: `session.create`, `session.start`, `session.stop`, `session.delete`. The table is range-partitioned monthly by `created_at`; `ensure_audit_log_partition(date)` materializes future partitions.
...
- **Webhook ingress (Phase 7):**
  ...
  - Webhook ingress is covered by the global IP floor introduced in Phase 11; a dedicated higher-burst limit class on `POST /api/webhooks/waha` can be added later if WAHA delivery patterns demand it.
- **Hardening (Phase 11):**
  - `helmet()` is reconfigured for an API-only stance: strict CSP (`default-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'none'`), HSTS (`max-age=HSTS_MAX_AGE_SECONDS; includeSubDomains; preload`), `Referrer-Policy: no-referrer`, COOP `same-origin`, CORP `same-site`, `X-Permitted-Cross-Domain-Policies: none`. A dedicated `Permissions-Policy` middleware denies camera, microphone, geolocation, payment, clipboard, USB, MIDI, sensors, fullscreen, autoplay, display-capture.
  - **Layered rate limits.** Two APP_INTERCEPTOR providers wire a single Redis sliding-window counter (`RateLimitService`, Lua `zremrangebyscore`/`zcard`/`zadd`+`pexpire`, `SCRIPT LOAD`+`evalsha` with `NOSCRIPT` recovery, fails open on Redis error). `GlobalRateLimitInterceptor` enforces IP (`RATE_LIMIT_IP_*`, default 600/min) and per-authenticated-user (`RATE_LIMIT_USER_*`, default 300/min) floors on every HTTP request. `RouteRateLimitInterceptor` reads `@RateLimit({preset, mode, identify, softCache?})` metadata and runs three presets today: `auth` (5/15min by email on `POST /api/auth/login`, hard), `send` (30/10s per user on `POST /api/messages/:chatId/send|media|forward`, hard), `chats` (soft 10/5s per user on `GET /api/chats` — serves the existing `CacheService.wrap` blob with `X-RateLimit-Cached: true`, degrades to a no-op when the cache misses). Hard 429s set `Retry-After`, `X-RateLimit-Limit`, `X-RateLimit-Remaining`. New `RateLimitedError` (`RATE_LIMITED`, 429).
  - **Request time budget.** `RequestTimeoutInterceptor` (global APP_INTERCEPTOR) wraps every HTTP handler in rxjs `timeout({ each: REQUEST_TIMEOUT_MS })` (default 30000) and surfaces `RequestTimeoutError` (`REQUEST_TIMEOUT`, 503). Non-HTTP contexts are skipped.
  - **CI gate.** `.github/workflows/ci.yml` adds `npm audit --omit=dev --audit-level=critical` as the hard gate plus an advisory `--audit-level=high` step (currently allows the transitive `multer` highs that only unwind on a NestJS 11 bump).
  - Live cross-pod limiter verification + securityheaders.com sweep land with the rest of the live-infra suite in Phase 12.

## 10. Testing baseline

Vitest runs three groups today (247 tests, all green):

1. **Unit** — env parsing (incl. PEM `\n` normalization), exception filter mapping, correlation middleware, snake-case helper, `SnakeNamingStrategy` hooks, `withTransaction` commit/rollback/isolation, `CacheService.wrap` (hit, miss, error path, lock contention fallback), `DistributedLockService` (acquire, retry, release, expired release), `AuthService` (login + refresh rotation + family revocation + change-password + logout + parseRefreshToken edge cases), `JwtAuthGuard` / `AdminGuard`, Zod auth schemas, `WorkerHarness` (Zod parse + correlation log fields + retry behaviour), `ExampleProcessor` / `ExampleQueueProducer` (jobId idempotency), `buildQueueOptions` (URL parsing, maxRetriesPerRequest, defaults), `TtlCache` (TTL expiry, stampede single-flight), `CircuitBreaker` (closed→open→half-open→closed state machine), `WahaClient` (axios round-trip + 5xx/4xx error wrapping against a local HTTP stub), `WahaService` (cache + retry + circuit + mutation no-retry + invalidation), `WahaStoreService` (read-only handle, rowid sort, JID lookups, bootstrap read-only check), `WebhooksService` (HMAC verify incl. sha256= prefix + tampered + missing inputs, enqueue jobId), `WebhookDispatch` (every event type routes, unknown ack-only, handler errors propagate), `WebhookProcessor` (phone→LID normalization across nested payloads, invalid envelope rejection), `WebhooksController` (HMAC gate + envelope plumbing), `PendingMessageStore` (TTL writes + isPending + resolve return value), `dedupeChats` (LID preference w/ phone↔LID aliasing), `sortByRowid` (null rowids sink, stable tiebreak), `ChatPolicy` (admin/developer stub), `SessionsService` (create/stop/applyStatusUpdate), `MessagesService.upsertFromWebhook` (pending-resolves-silently vs novel-emits-message:new), `UsersService` (duplicate-email rejection, bcrypt hash + audit, disable revokes families + disconnects sockets, enable is no-op for tokens/sockets, password reset disconnects), `AssignmentsService` (missing-user rejection, partial-unique conflict, atomic history+emit, deactivation lifecycle), `MuteService` (admin always allowed, developer requires active assignment, global toggle audits), `FeedbackService` (developer self-scoped list, admin sees everything, admin-only mark-read), `ChatPolicy` (assignment-driven visibility + read/write gates), Phase 10 metrics registry (registered metric set matches the doc), `MetricsInterceptor` (matched-route labelling + error-status branch), `@TraceMethod` decorator (sync + async + rethrow), Phase 11 `RateLimitService` (sliding window correctness, window slide, per-key isolation, Redis-down fail-open), `GlobalRateLimitInterceptor` (IP-only / IP+user / hard 429 + `Retry-After` / env-disabled noop), `RouteRateLimitInterceptor` (no descriptor passthrough, hard 429, soft hit serves cached, soft miss degrades, env-disabled noop), `RequestTimeoutInterceptor` (fast passthrough, timeout → 503, handler-error passthrough, non-HTTP skip, envelope sanity).
2. **Integration** (`test/health.e2e-spec.ts`, `test/realtime.e2e-spec.ts`, `test/security.e2e-spec.ts`) — boot minimal Nest apps that exercise the live probe + 404 envelope, the Socket.IO handshake + room behaviour, and the Phase 11 helmet posture + `RequestTimeoutInterceptor` 503 envelope without requiring real Postgres / Redis. End-to-end Testcontainers Postgres + Redis + WAHA-stub coverage (full login → refresh → logout, send → reconcile, queue round-trip, cross-pod adapter fanout, cross-pod rate-limit shared counter) is deferred to Phase 12.
...
| 7 — chats | `GET /api/chats?session=<n>`, `POST /api/chats/:chatId/read`, `POST /api/chats/sync?session=<n>` — live. Mute toggles ship in Phase 9 under `/api/mute`. | 8 — Domain (✅) |
...
| 9 — admin | Live: `GET /api/admin/users`, `GET /api/admin/users/:id`, `POST /api/admin/users`, `PATCH /api/admin/users/:id`, `POST /api/admin/users/:id/disable`, `POST /api/admin/users/:id/enable`, `POST /api/admin/users/:id/password-reset`, `DELETE /api/admin/users/:id`; `GET /api/admin/assignments`, `POST /api/admin/assignments`, `DELETE /api/admin/assignments/:id`, `GET /api/admin/assignments/:id/history`; `POST /api/mute/chat`, `POST /api/mute/global`, `GET /api/mute`; `POST /api/feedback`, `GET /api/feedback`, `POST /api/feedback/:id/read`. | 9 — Collaboration (✅) |
...
Socket events the frontend will register handlers for (phase-ordered, names from `FRONTEND_ARCHITECTURE.md §6` and `FRONTEND_IMPLEMENTATION_PLAN.md` phases 7–9): `message:new`, `message:ack`, `message:edited`, `message:deleted`, `message:reaction`, `session:status`, `group:participants` (all live as of Phase 8); `chat:assigned`, `chat:unassigned` (live as of Phase 9 — sent to `user:<dev>` + `admin` rooms on every assignment mutation); `chat:read`, `chat:muted` (planned — assignment of mute toggle to a socket event was not required to land Phase 9); `auth:ready`, `auth:logged-out` (planned). Mirror the payload Zod schemas in `realtime/events.contract.ts` on the frontend side; both ends must agree.
...
| Area | Doc | Status |
| --- | --- | --- |
| Bootstrap (`main.ts`, `AppModule`) | [modules/bootstrap.md](modules/bootstrap.md) | ✅ Phases 1–11 wiring (strict helmet + APP_INTERCEPTORs landed Phase 11) |
| ConfigModule + env parsing | [modules/config.md](modules/config.md) | ✅ Phases 1–11 surface |
| LoggerModule (`nestjs-pino`) | [modules/logger.md](modules/logger.md) | ✅ Phase 1 |
| Common (middleware, filter, pipe, decorators, guards) | [modules/common.md](modules/common.md) | ✅ Phase 1 + 3 + 11 (interceptors) |
| Security hardening (rate limit + request timeout + helmet) | [modules/security.md](modules/security.md) | ✅ Phase 11 |
| Shared (errors, branded IDs, Result, Express types) | [modules/shared.md](modules/shared.md) | ✅ Phase 1 |
| DatabaseModule (TypeORM, naming, transactions, migrations, partitions) | [modules/db.md](modules/db.md) | ✅ Phase 2 + 3 + 8 + 9 (migration 0004 collaboration) |
...
| UsersModule | [modules/users.md](modules/users.md) | ✅ Phase 3 + 9 (admin CRUD) |
| AuthModule | [modules/auth.md](modules/auth.md) | ✅ Phase 3 + 9 (audit surface) |
| AssignmentsModule | [modules/assignments.md](modules/assignments.md) | ✅ Phase 9 |
| MuteModule | [modules/mute.md](modules/mute.md) | ✅ Phase 9 |
| FeedbackModule | [modules/feedback.md](modules/feedback.md) | ✅ Phase 9 |
| MetricsModule (prom-client + OTel + health probes) | [modules/observability.md](modules/observability.md) | ✅ Phase 10 |
| RealtimeModule | [modules/realtime.md](modules/realtime.md) | ✅ Phase 4 + 8 + 9 events |
...
| ChatsModule | [modules/chats.md](modules/chats.md) | ✅ Phase 8 + 9 (assignment-gated visibility, mute enrichment) |
